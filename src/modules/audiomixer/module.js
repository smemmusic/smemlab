...
// 4-channel audio summing mixer. Same shape as the CV mixer but with audio
// ports — per-channel gain (dB) + phase reverse, master volume:
//
//   in1 → ch1Gain ──┐
...
// anything ≤ -60 → linear 0 (true silence at "−∞"). Phase reverse is genuinely
// useful on audio: flipping one of two near-identical waves cancels them
// instead of summing.
export class AudioMixerModule extends AudioModule {
  static KIND = MODULE_KIND.AUDIO;
  static PORTS = [
    { name: "in1", dir: PORT_DIR.IN,  type: PORT_TYPE.AUDIO },
    { name: "in2", dir: PORT_DIR.IN,  type: PORT_TYPE.AUDIO },
    { name: "in3", dir: PORT_DIR.IN,  type: PORT_TYPE.AUDIO },
    { name: "in4", dir: PORT_DIR.IN,  type: PORT_TYPE.AUDIO },
    { name: "out", dir: PORT_DIR.OUT, type: PORT_TYPE.AUDIO },
  ];
  static CONTROLS = [
  ...
  constructor(ctx, params = {}) {
    ...
    for (let i = 0; i < 4; i++) this._registerAudioIn(`in${i + 1}`, this.channels[i]);
    this._registerAudioOut("out", this.master);
